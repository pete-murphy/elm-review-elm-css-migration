module Css.Migration.OnlyInAttributeList exposing (rule)
...
{-| Reports... REPLACEME
    ...
    config =
        [ Css.Migration.OnlyInAttributeList.rule
        ]
...
```bash
elm-review --template pete-murphy/elm-review-elm-css-migration/example --rules Css.Migration.OnlyInAttributeList
```
...
isCssAttribute : Node Expression -> Bool
isCssAttribute node =
    case node of
        Node _ (Expression.FunctionOrValue [ "Html", "Styled", "Attributes" ] "css") ->
            True
...
expressionEnterVisitor node context =
    case node of
        Node range (Expression.FunctionOrValue [ "Html", "Styled", "Attributes" ] "css") ->
            case context.ignoredRange of
                Just _ ->
                    ( [ Rule.errorForModule context.moduleKey
                            { message = "Found Css module"
                            , details = []
                            }
                            range
                      ]
